---
translationKey: ressources
lang: fr
createdAt: 2025-11-14T09:44:00.000Z
uuid: 3bfd46874867
localizationKey: d23345a84392
name: Ressources
eleventyNavigation:
  order: 6
---

{% partialWrapper "page-header.njk", { image: "/_images/pexels-rethaferguson-3810762.webp", class: "palette-burnt-umber" } %}

# Ressources

Vous souhaitez renforcer vos projets ou approfondir vos pratiques?
Découvrez nos ressources documentaires et partenaires de terrain actifs dans la promotion de la santé et la formation. Des outils, des recherches, des réseaux: tout ce qu’il faut pour soutenir vos démarches et vos actions.

{% endpartialWrapper %}

::: section { .bleed-bg .palette-cooper .palette-low-contrast }

## Documentation

- nom document - thématique
- nom document - thématique
- nom document - thématique
- nom document - thématique

:::

:::::: section { .palette-burnt-umber }

## Vous recherchez un soutien méthodologique? { .width-max }

### Consultez le répertoire pour plus d’informations{ .width-max .mt-[var(--step--2)]}

#### À Bruxelles

::::: div { .section .fixed-fluid .items-center }

<a href="/assets/files/repertoire-servicessupportpromosantebxl_2026.pdf" target="_blank" rel="noopener noreferrer">
{% image src="/_images/repertoire-servicessupportpromosantebxl_2026.webp", width="300" %}
</a>

::: div { .flow }

**Répertoire de l’offre d’accompagnement et de support en promotion de la santé en région bruxelloise** texte à rédiger…

<p>
<a href="/sesu/#decouvrez-les-autres-sesu" class="button palette-orange-peel-contrast">
Consultez le répertoire des SESU
</a>
</p>

:::
:::::

#### En Wallonie

::::: div { .section .fixed-fluid .items-center }

<a href="https://lesclps.org/" target="_blank" rel="noopener noreferrer">
{% image src="/_images/logo-interclps.webp", width="300" %}
</a>

::: div

**Les Centres Locaux de Promotion de la Santé** (CLPS) sont des organismes qui soutiennent et accompagnent les professionnels locaux dans la mise en place de projets visant à améliorer la santé et le bien-être des populations. Ils offrent un appui méthodologique, facilitent la mise en réseau des acteurs de terrain et organisent des formations et des échanges de pratiques.

<a href="https://lesclps.org/" target="_blank" rel="noopener noreferrer">
>lien<
</a>
:::
:::::

::::: div { .section .fixed-fluid .items-center }

<a href="https://www.aviq.be/fr/sensibilisation-et-promotion/operateurs-de-promotion-de-la-sante/centres-dexpertises" target="_blank" rel="noopener noreferrer">
{% image src="/_images/logo_aviq_officiel.svg", width="300" %}
</a>

::: div

**Les Centres d’expertise** en promotion de la santé sont agréés en vue d’apporter un support scientifique et méthodologique à l’Agence, aux centres locaux de promotion de la santé, aux centres d’opérationnalisation en médecine préventive et aux opérateurs en promotion de la santé

<a href="https://www.aviq.be/fr/sensibilisation-et-promotion/operateurs-de-promotion-de-la-sante/centres-dexpertises" target="_blank" rel="noopener noreferrer" class="mx-auto">
>lien<
</a>
:::
:::::

::::::

::::: section { .bleed-bg .palette-cooper .palette-low-contrast }

### Vous recherchez un soutien politique? { .width-max }

Les fédés de promotion de la santé avec les sites web. { .width-max .mt-[var(--step--2)]}

::: div { .mt-[var(--step-2)] .flow .horizontal .mx-0 .gap-[var(--step-4)]}

<a href="https://www.fbpsante.brussels/" target="_blank" rel="noopener noreferrer">
{% image src="/_images/fbps-logo-1.webp", width="300" %}
</a>

<a href="https://www.fwpsante.be/" target="_blank" rel="noopener noreferrer">
{% image src="/_images/fwps-logo-colors-01.webp", width="300" %}
</a>

:::

:::::
